...
| `Red Hat Lightwell` | The Red Hat Lightwell ecosystem; Lightwell is an automated remediation pipeline that backports (or, for novel vulnerabilities, generates) fixes for open source Java libraries and publishes the patched, signed artifacts. The `name` field is a Maven package name in the format `groupId:artifactId`. |
