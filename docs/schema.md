...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>DHI</code></td>
      <td><a href="https://github.com/docker-hardened-images/advisories">Docker Hardened Images Advisories</a></td>
      <td>
        <ul>
          <li>How to contribute: TBD</li>
          <li>Source URL: <code>https://github.com/docker-hardened-images/advisories</code></li>
    ...
    <tr>
      <td><code>VCPKG</code></td>
      <td><a href="https://github.com/microsoft/vcpkg">vcpkg Security Advisory</a></td>
      <td>
        <ul>
          <li>How to contribute: TBD</li>
          <li>Source URL: <code>TBD</code></li>
...
| `Echo` | The Echo package ecosystem; the `name` is the name of the source package. The ecosystem string might optionally have a `:<ECOSYSTEM>` suffix to denote an Echo secured build of a package from another ecosystem, in which case the `name` is the name of the package within that ecosystem and its versions are ordered by that ecosystem's rules. The supported values are `Echo:PyPI`, `Echo:Maven` and `Echo:npm`. Without a suffix the package is an Echo OS package, whose versions are ordered by the Debian version ordering. Echo's secured builds carry a `+echo.<N>` build suffix, e.g. `1.2.3+echo.1`. |
...
| `WordPress` | The WordPress ecosystem. The ecosystem string may have a suffix to specify the component: `:Core` for WordPress Core (the `name` is `wordpress`), `:Plugin` for plugins (the `name` is the plugin slug), or `:Theme` for themes (the `name` is the theme slug). The slug is the unique identifier used in the WordPress.org repository. |
